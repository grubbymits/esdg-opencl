...
/// LE1ToolChain - A tool chain using the llvm bitcode tools to perform
/// all subcommands.
/// Currently does not support anything else but compilation.

LE1ToolChain::LE1ToolChain(const Driver &D, const llvm::Triple& Triple)
  : ToolChain(D, Triple) {
  // Path mangling to find libexec
  std::string Path(getDriver().Dir);
...
/*
bool LE1ToolChain::IsUnwindTablesDefault() const {
  return false;
}

const char *LE1ToolChain::GetDefaultRelocationModel() const {
  return "static";
}

const char *LE1ToolChain::GetForcedPicModel() const {
  return 0;
}*/
...
Tool &LE1ToolChain::SelectTool(const Compilation &C,
  ...
  if (!T) {
    ...
    case Action::AnalyzeJobClass:
      T = new tools::Clang(*this); break;
    default:
     llvm_unreachable("Unsupported action for LE1 target.");
    }
  }
  return *T;
}
