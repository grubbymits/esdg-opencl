...
void _clInit()
{
  ...
  oclHandles.cl_status = clGetProgramInfo(oclHandles.program,
...
void _clInvokeKernel(int kernel_id, int work_items, int work_group_size)
  ...
  std::cout << "Workgroup size " << work_group_size << " and global "
    << work_items;
  oclHandles.cl_status = clEnqueueNDRangeKernel(oclHandles.queue,
                                                oclHandles.kernel[kernel_id],
