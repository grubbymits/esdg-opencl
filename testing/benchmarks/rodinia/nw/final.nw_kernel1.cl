...
index_nw =  base + cols *  16 /*BLOCK_SIZE*/ * b_index_y +  16 /*BLOCK_SIZE*/ * b_index_x;
	...
	if (tx[__esdg_idx] == 0){
		 input_itemsets_l [ 0 + tx[__esdg_idx] * ( 16 + 1 ) ] /*SCORE*/ /*(tx, 0)*/ = input_itemsets_d[index_nw + tx[__esdg_idx]];
	}
...
for (__esdg_idx = 0; __esdg_idx < 16; ++__esdg_idx) {
   ...
   for ( int ty = 0 ; ty <  16 /*BLOCK_SIZE*/ ; ty++)
     input_itemsets_d[index[__esdg_idx] + cols * ty] =  input_itemsets_l [ ( tx[__esdg_idx] + 1 ) + ( ty + 1 ) * ( 16 + 1 ) ] /*SCORE*/ /*((ty+1), (tx+1))*/;
