    ...
    for (int d = 0; d < 16; d++)
    {
        ...
        // Exclusive scan the counts in local memory
         ...
         int exclusive = 1;
             ...
             {
                 t = lmem_arg[idx -  i]; barrier( 1  /*CLK_LOCAL_MEM_FENCE*/);
                 lmem_arg[idx] += t;     barrier( 1  /*CLK_LOCAL_MEM_FENCE*/);
             }
             __trans_tmp_1 =  lmem_arg[idx-exclusive];
         }
         
         int /*FPTYPE*/ res = __trans_tmp_1;
            ...
            const int shift)
{
    ...
    __local  int /*FPTYPE*/ l_scanned_seeds[16];
    
    // Keep a shared histogram of all instances seen by the current
    // block
    __local  int /*FPTYPE*/ l_block_counts[16];
    ...
    __global  uint4 /*FPVECTYPE*/ *in4  = (__global  uint4 /*FPVECTYPE*/*) in;
    ...
    while (window < block_stop)
    {
        ...
        for (int q = 0; q < 16; q++) histogram[q] = 0;
         uint4 /*FPVECTYPE*/ val_4;
         uint4 /*FPVECTYPE*/ key_4;        
        ...
        for (int digit = 0; digit < 16; digit++)
        {
            ...
            int exclusive = 1;
                ...
                __trans_tmp_2 =  lmem_arg[idx-exclusive];
            }
            
            histogram[digit] = __trans_tmp_2;
            barrier( 1  /*CLK_LOCAL_MEM_FENCE*/);
        }
