...
void test_kernel(int* input, 
  ...
  for (i[__kernel_local_id[0]] = 0; i[__kernel_local_id[0]] < 32;
       ++i[__kernel_local_id[0]]) {
    ...
    for (j[__kernel_local_id[0]] = 0;
         j[__kernel_local_id[0]] < i[__kernel_local_id[0]];
         ++j[__kernel_local_id[0]]) {

      __kernel_local_id[0] = 0;
      while (__kernel_local_id[0] < 16) {
        result[gid[__kernel_local_id[0]]] = input[gid[__kernel_local_id[0]]] *
          input[gid[__kernel_local_id[0]] + j[__kernel_local_id[0]]];
...
/*
result[15]  = input[15] * input[15 + 31];
            = 15 * 46
            = 690
*/
