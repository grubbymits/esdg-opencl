...
cl_command_queue OpenCL::queue()
{
	return this->command_queue;
}

void OpenCL::launch(std::string toLaunch)
{
...
cl_kernel OpenCL::kernel(std::string kernelName)
{
	return this->kernelArray[kernelName];
}

void OpenCL::createKernel(std::string kernelName)
{
  ...
  clGetKernelWorkGroupInfo(kernelArray[kernelName], device_id[0],
                           ...
                           NULL);
  if (lwsize == 0) {
    std::cout << "Error: max work group size of zero!" << std::endl;
    exit(1);
  }
...
void OpenCL::buildSource(const char *filename)
{
  std::cout << "OpenCL::buildKernel\n";
	...
	if (ret != CL_SUCCESS)
	{
	  printf("\nError at clBuildProgram! Error code %i\n\n", ret);
          std::cout << "\n*************************************************"
            << std::endl;
          std::cout << "***   OUTPUT FROM COMPILING THE KERNEL FILE   ***"
            << std::endl;
          std::cout << "*************************************************"
            << std::endl;
	  ...
	  std::cout << "\n*************************************************"
            << std::endl;
	  std::cout << "*** END OUTPUT FROM COMPILING THE KERNEL FILE ***"
            << std::endl;
	  std::cout << "*************************************************\n\n"
            << std::endl;
	  exit(1);
