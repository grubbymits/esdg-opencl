...
namespace {
  struct myLICM : public LoopPass {
    static char ID; // Pass identification, replacement for typeid
    myLICM() : LoopPass(ID) {
	//initializemyLICMPass(*PassRegistry::getPassRegistry());
    }
    ...
    virtual bool doInitialization(Loop * myloop, LPPassManager &LPM) {
	...
	for (Loop::block_iterator loopIter = myloop->block_begin(), E = myloop->block_end(); loopIter != E; loopIter++ )
        {	
                if((*loopIter)->getName() == "bb3") //if instruction that produces operand is in superblock, return false
		{
			header = (*loopIter);
		}
		if((*loopIter)->getName() == "bb2")
		{
			blockList.push_back(*loopIter);	
			bb2 = (*loopIter);	
		}
		if((*loopIter)->getName() == "bb")
		{
			blockList.push_back(*loopIter);
			bb = (*loopIter);		
		}
	...
	superBlocks[header] = blockList;
	partOfSuperBlock[header] = header;
	partOfSuperBlock[bb] = header;
	partOfSuperBlock[bb2] = header;	
  ...
  private:
    ...
    //maps each header BB of a superblock (which represents a superblock) to
    //it's "hoist block", the BB right before the preheader that is created
    //to hold all the superblock invariant instructions 
    map<BasicBlock*, BasicBlock*> superBlockToHoistBlock;
...
char myLICM::ID = 0;
/*INITIALIZE_PASS_BEGIN(myLICM, "myLicm", "My Loop Invariant Code Motion", false, false)
INITIALIZE_PASS_DEPENDENCY(DominatorTree)
INITIALIZE_PASS_DEPENDENCY(LoopInfo)
INITIALIZE_PASS_DEPENDENCY(LoopSimplify)
INITIALIZE_AG_DEPENDENCY(AliasAnalysis)
INITIALIZE_PASS_END(myLICM, "myLicm", "My Loop Invariant Code Motion", false, false)*/

static RegisterPass<myLICM> Z("myLICM", "my LICM", false, false);

//Pass *llvm::createLICMPass() { return new myLICM(); }

/// Hoist expressions out of the specified loop. Note, alias info for inner
/// loop is not preserved so it is not a good idea to run LICM multiple 
/// times on one loop.
///
bool myLICM::runOnLoop(Loop *L, LPPassManager &LPM) {
  errs() << "INSIDE OF RUNONLOOP\n\n\n\n\n\n";
  Changed = false;
  ...
  if(SPPreheaderSet) //if there is a superblock preheader
  {
      ...
      for (pred_iterator pred_b = pred_begin(CurLoop->getHeader()), pred_e = pred_end(CurLoop->getHeader());
          ...
          if (    ((partOfSuperBlock).find(*pred_b) == (partOfSuperBlock).end()) 
               || ((partOfSuperBlock).find(*pred_b)->second != CurLoop->getHeader())) {
...
/// iteration.
///
void myLICM::SinkRegion(DomTreeNode *N) {
  ...
  for (BasicBlock::iterator II = BB->end(); II != BB->begin(); ) {
    ...
    if (isInstructionTriviallyDead(&I)) {
      DEBUG(dbgs() << "myLICM deleting dead inst: " << I << '\n');
      ++II;
...
///
void myLICM::HoistRegion(DomTreeNode *N) {
  assert(N != 0 && "Null dominator tree node?");
  BasicBlock *BB = N->getBlock();
  ...
  if (!inSubLoop(BB))
    for (BasicBlock::iterator II = BB->begin(), E = BB->end(); II != E; ) {
      ...
      if (Constant *C = ConstantFoldInstruction(&I)) {
        DEBUG(dbgs() << "myLICM folding inst: " << I << "  --> " << *C << '\n');
      ...
      else if (canSinkOrHoistInst(I) && isSafeToExecuteUnconditionally(I) && 
          invariantInSuperBlock(I)) 
      {
        //if instruction doesnt have loop invariant operands but is part of a 
...
/// instruction.
///
bool myLICM::canSinkOrHoistInst(Instruction &I) {
...
///
bool myLICM::isNotUsedInLoop(Instruction &I) {
...
///
void myLICM::sink(Instruction &I) {
  DEBUG(dbgs() << "myLICM sinking instruction: " << I << "\n");
...
/// that is safe to hoist, this instruction is called to do the dirty work.
///
void myLICM::hoist(Instruction &I) {
  DEBUG(dbgs() << "myLICM hoisting to " << Preheader->getName() << ": "
        << I << "\n");
...
void myLICM::hoistSPInvariantInst(Instruction &I)
{
...
bool myLICM::invariantInSuperBlock(Instruction &I)
{
    ...
    if ((partOfSuperBlock).find(bbOfInst) == (partOfSuperBlock).end()) {
      return false;
    }

    headBB = (partOfSuperBlock).find(bbOfInst)->second;
    

    list<BasicBlock*> trace = superBlocks[headBB];
    ...
    //for every source operand, go through all instructions in superblock
    for (unsigned i = 0, e = I.getNumOperands(); i != e; ++i)
    {
        if (Instruction *I_operand = dyn_cast<Instruction>(I.getOperand(i))) //FIXME, dont know whether im getting the right pointer
        {
            //if operand is an instruction, then look through every other
            //instruction in superblock and return false if there is a match

	  	for(traceIter=trace.begin() ; traceIter != trace.end(); traceIter++)
            	{        
	              for (BasicBlock::iterator bbIter = (*traceIter)->begin(), E = (*traceIter)->end(); bbIter != E; bbIter++ )
	              {	
		                if(I_operand == bbIter) //if instruction that produces operand is in superblock, return false
                    		{
                        		return false;
                    		}
	              }	
        ...
        //if the dyn_cast fails then that means operand is not an instruction
        //so that operand is invariant, move on to check other operands      
    }  
...
/// guaranteed to execute.
///
bool myLICM::isSafeToExecuteUnconditionally(Instruction &Inst) {
...
///
void myLICM::PromoteAliasSet(AliasSet &AS) {
  ...
  DEBUG(dbgs() << "myLICM: Promoting value stored to in loop: " <<*SomePtr<<'\n');  
  Changed = true;
  ++NumPromoted;
...
/// cloneBasicBlockAnalysis - Simple Analysis hook. Clone alias set info.
void myLICM::cloneBasicBlockAnalysis(BasicBlock *From, BasicBlock *To, Loop *L) {
...
/// deleteAnalysisValue - Simple Analysis hook. Delete value V from alias
/// set.
void myLICM::deleteAnalysisValue(Value *V, Loop *L) {
  AliasSetTracker *AST = LoopToAliasSetMap.lookup(L);
  if (!AST)
    return;
