...
KernelArg* create_kernel_arg(const char* mem, const char* ty, unsigned num) {
  KernelArg* arg = new KernelArg();
  char datatype = 0;

  // Fix if the argument is defined as a u* instead of 'unsigned'
  if(ty[0] == 'u')
    datatype = ty[1];
  else
    datatype = ty[0];
...
int preprocess(char* filename) {
        ...
        do {
          ...
          if(token.getKind() == tok::r_paren)
            ...
            if(arg_attrs >= NUM_ARG_ATTRS) {
              args.push_back(create_kernel_arg(attrs[0], attrs[1],
                                               args.size()));
              arg_attrs = 0;
            }
          }
  ...
  for(std::list<KernelArg*>::iterator ArgIt = args.begin();
    ...
    std::cout << "Size of data type = " << (*ArgIt)->size << std::endl;
    delete (*ArgIt);
  }
