...
bool WorkitemCoarsen::CreateWorkgroup(const char *filename) {
  ...
  InitKernel = std::string(RewriteBuf->begin(), RewriteBuf->end());
  /*
  std::ofstream init_kernel;
  InitFilename = std::string("init.");
  InitFilename.append(OrigFilename);
  init_kernel.open(InitFilename.c_str());
  init_kernel << source;
  init_kernel.close();*/
...
bool WorkitemCoarsen::Compile(std::string &filename, std::string &source,
                              std::string &triple) {
  if (!ProduceBytecode(filename, source, triple))
    return false;
...
bool WorkitemCoarsen::ProduceBytecode(std::string &filename,
  ...
  TheCompiler.getTargetOpts().Triple = triple;
